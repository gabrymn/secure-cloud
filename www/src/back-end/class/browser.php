<?php
    ...
    function get_browser_info($http_user_agent)
    {
        ...
        if($http_user_agent == 'Mozilla/5.0(iPad; U; CPU iPhone OS 3_2 like Mac OS X; en-us) AppleWebKit/531.21.10 (KHTML, like Gecko) Version/4.0.4 Mobile/7B314 Safari/531.21.10') {
            $deviceType='Tablet';
        }

        if(stristr($http_user_agent, 'Mozilla/5.0(iPad;')) {
            $deviceType='Tablet';
        }
        ...
        $browserInfo['user_agent'] = $u_agent;
        $browserInfo['browser'] = $bname;
        $browserInfo['browser_version'] = $version;
        $browserInfo['os_platform'] = $platform;
        $browserInfo['pattern'] = $pattern;
        $browserInfo['device'] = $deviceType;
